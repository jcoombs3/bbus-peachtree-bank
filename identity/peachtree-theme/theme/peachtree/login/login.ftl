...
<@layout.registrationLayout displayInfo=social.displayInfo displayWide=(realm.password && social.providers??); section>
    <#if section = "header">
        <#if realm.name = "master">
            ${msg("logInHeader")}
        <#else>
            <div class="peachtree-header">
            <div class="peachtree-logo"></div>
        ...
        <div id="kc-form-wrapper" <#if realm.password && social.providers??>class="${properties.kcFormSocialAccountContentClass!} ${properties.kcFormSocialAccountClass!}"</#if>>
            <#if realm.password>
                <form id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                    ...
                    <div class="${properties.kcFormGroupClass!} password-input-container">
                        <label for="password" class="${properties.kcLabelClass!}">${msg("password")}</label>
                        <input id="password" class="${properties.kcInputClass!}" name="password" type="password" autocomplete="off" />
                         <button type="button" id="password-visibility-toggle-button" tabindex="3" aria-labelledby="show-password-aria-label">
                        <i id="password-eye-icon" class="fa fa-eye" aria-hidden="true"></i>
                    </button>
                    </div>
